...
CoreGui:SetCore("SendNotification", {
    Title = "NeoBlox Service Arsenal V1 Premium",
...
local Window     = Library.CreateLib("AdvanceTech | Arsenal | v1.9", "BlueTheme")
 local Window     = Library.CreateLib("NeoBlox Service | Arsenal V1", "BlueTheme")

local Welcome     = Window:NewTab("Main")
-- ...existing code...
 local MainSection = Welcome:NewSection("Welcome To NeoBlox Service | " .. game.Players.LocalPlayer.Name)
local HitboxSection = Welcome:NewSection("> Hitbox Settings <")

local AimbotTab = Welcome:NewSection("> Aimbot <")
local _ = AimbotTab:NewLabel("AimBot Beta")
...
AimbotTab:NewToggle("Enable Aimbot", "Toggle aimbot on/off", function(state)
    aimbotActive = state
end)

AimbotTab:NewSlider("Aimbot FOV", "Adjust aimbot FOV", 300, 10, function(val)
    aimbotFOV = val
end)

AimbotTab:NewDropdown("Target Part", "Choose body part to aim at", {"Head", "UpperTorso", "LowerTorso", "HumanoidRootPart"}, function(part)
    aimbotTargetPart = part
end)

AimbotTab:NewToggle("Team Check", "Only aim at enemies", function(state)
...
local function getClosestPlayer()
    ...
    local closestPlayer, closestDist = nil, aimbotFOV
    for _, player in ipairs(game.Players:GetPlayers()) do
        if player ~= localPlayer and player.Character and player.Character:FindFirstChild("Humanoid") and player.Character:FindFirstChild(aimbotTargetPart) then
...
RunService.RenderStepped:Connect(function()
    if aimbotActive and aiming then
...
local _ = SilentTab:NewLabel("Silent Aim Beta")
...
WarningText.Text = "" -- made it into empty string, you can add whatever
 WarningText.TextColor3 = Color3.fromRGB(245, 222, 179) -- Besch
...
local ao = Color3.new(50, 50, 50)
 Random:NewColorPicker('Arm Color', "?", Color3.fromRGB(245, 222, 179), function(ap)
  ao = ap
end)
...
local au = Color3.new(50, 50, 50)
 Random:NewColorPicker('Gun Color', "?", Color3.fromRGB(245, 222, 179), function(ap)
  au = ap
end)
local av = false;
...
local Section = Credit:NewSection("Script Developed by: NeoBlox Service")
...
Section:NewButton("Copy Discord Link", "Discord", function()
    setclipboard("https://discord.gg/XjKUB2GyqB")
end)
...
frame.ImageColor3 = Color3.fromRGB(11, 18, 7)
 frame.ImageColor3 = Color3.fromRGB(245, 222, 179) -- Besch
...
button.TextColor3 = Color3.fromRGB(0, 34, 255)
 button.TextColor3 = Color3.fromRGB(245, 222, 179) -- Besch
 local Section = Credit:NewSection("Script Developed by: NeoBlox Service")
button.TextSize = 20
